...
void GS_Main(point VSInput input[1], inout TriangleStream<GSOutput> TriStream)
{
	...
	// Top left.
	//
	output.Position = mul(input[0].Matrix, float4(0, 0, 0, 1));
	...
	// Bottom left.
	//
	output.Position = mul(input[0].Matrix, float4(0, 1, 0, 1));
...
{
	return saturate(tex0.Sample(sampler0, input.Texture0) * input.Color);
}
...
void main()
{
	...
	// Top right.
	//
	gl_Position = gvMatrix[0] * vec4(1, 0, 0, 1);
	vTex0 = vec2(gvTex0[0].z, gvTex0[0].y);
	EmitVertex();

	// Top left.
	//
	gl_Position = gvMatrix[0] * vec4(0, 0, 0, 1);
