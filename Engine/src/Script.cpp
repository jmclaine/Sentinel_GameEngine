...
#define PUSH_VAR(a)\
	{\
		...
		else if (v0->type == VALUE_STRING)\
		{\
			STRING_ALLOC()\
			v0->str->incRef();\
		}\
		else if (v0->type == VALUE_VECTOR)\
		{\
			VECTOR_ALLOC()\
			v0->vec->incRef();\
		}\
...
#define STORE(a)\
	...
	int Script::Load(const char* filename)
	{
		...
		Debug::Log(STREAM("Loading Script: " << filename));

		int i = comp.Load(filename);

		Debug::Log((i != 0) ? "Success!" : "Failed!");
...
#define OVERRIDE_STACK(type)\
	...
	void Value::Assign(Value& right)
	{
		...
		else if (right.type == VALUE_VECTOR)
		{
			VECTOR_ALLOC()
			right.vec->incRef();
		}
