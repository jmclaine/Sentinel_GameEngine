...
namespace Sentinel
{
	namespace Component
	{
		SerialRegister PerspectiveCamera::SERIAL_REGISTER("PerspectiveCamera", PerspectiveCamera::Clone);
		Serializable* PerspectiveCamera::Clone() { return new PerspectiveCamera(); }
		...
		Ray PerspectiveCamera::ScreenPointToRay(UINT mouseX, UINT mouseY, UINT screenWidth, UINT screenHeight)
		{
			...
			if (screenWidth == 0)
				screenWidth = WindowInfo::BASE_WIDTH;

			if (screenHeight == 0)
				screenHeight = WindowInfo::BASE_HEIGHT;
		...
		void PerspectiveCamera::Copy(GameComponent* component)
		{
			Camera::Copy(component);

			auto camera = static_cast<PerspectiveCamera*>(component);
